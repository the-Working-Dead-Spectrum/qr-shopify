...
<head>
    ...
    <title>@yield('title', 'Paramètres Système') — {{ config('app.name') }}</title>
...
<body class="bg-surface text-on-surface flex min-h-screen overflow-hidden">
    ...
    <aside class="hidden md:flex flex-col h-screen py-lg px-md bg-surface-container-low dark:bg-surface-container-lowest border-r border-outline-variant w-64 shrink-0">
        ...
        <nav class="flex-1 space-y-xs">
            <a class="flex items-center gap-md px-md py-sm rounded-lg text-on-surface-variant dark:text-surface-variant hover:bg-surface-container-high dark:hover:bg-surface-container-highest transition-colors font-body-md text-body-md" href="{{ route('admin.dashboard') }}">
                <span class="material-symbols-outlined">dashboard</span>
                <span>Tableau de bord</span>
            </a>
            <a class="flex items-center gap-md px-md py-sm rounded-lg text-on-surface-variant dark:text-surface-variant hover:bg-surface-container-high dark:hover:bg-surface-container-highest transition-colors font-body-md text-body-md" href="{{ route('admin.partners.index') }}">
                <span class="material-symbols-outlined">handshake</span>
                <span>Partenaires</span>
            </a>
            <a class="flex items-center gap-md px-md py-sm rounded-lg text-on-surface-variant dark:text-surface-variant hover:bg-surface-container-high dark:hover:bg-surface-container-highest transition-colors font-body-md text-body-md" href="{{ route('admin.orders.index') }}">
                <span class="material-symbols-outlined">shopping_cart</span>
                <span>Commandes</span>
            </a>
            <a class="flex items-center gap-md px-md py-sm text-primary dark:text-primary-fixed font-bold border-r-4 border-primary bg-surface-container-highest font-body-md text-body-md" href="{{ route('admin.settings') }}">
                <span class="material-symbols-outlined">settings</span>
                <span>Paramètres</span>
            </a>
            <a class="flex items-center gap-md px-md py-sm rounded-lg text-on-surface-variant dark:text-surface-variant hover:bg-surface-container-high dark:hover:bg-surface-container-highest transition-colors font-body-md text-body-md" href="{{ route('admin.logs') }}">
        ...
        <div class="mt-auto pt-md border-t border-outline-variant flex items-center gap-sm">
            ...
            <div class="overflow-hidden">
                <p class="text-body-md font-bold truncate">{{ auth()->user()->name }}</p>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-lg">
                    <div class="lg:col-span-2 space-y-lg">
                        <div class="bg-surface-container-lowest p-lg rounded-xl border border-outline-variant shadow-sm">
                            ...
                            <form method="POST" action="{{ route('admin.settings.test-shopify-connection') }}" class="space-y-md">
                                @csrf
                                <div class="space-y-xs">
                                    ...
                                    <input class="w-full px-md py-sm rounded-lg border border-outline-variant focus:ring-2 focus:ring-tertiary focus:border-primary outline-none transition-all" 
                                           ...
                                           type="text" 
                                           name="shop_domain" 
                                           value="{{ config('shopify.shop_domain') ?? '' }}" required>
                                </div>
                                <div class="space-y-xs">
                                    ...
                                    <div class="relative">
                                        <input class="w-full px-md py-sm pr-12 rounded-lg border border-outline-variant focus:ring-2 focus:ring-tertiary focus:border-primary outline-none transition-all" 
                                               type="password" 
                                               name="api_key" 
                                               value="{{ config('shopify.api_key') ?? '' }}" required>
                        ...
                        <div class="bg-surface-container-low p-lg rounded-xl border border-outline-variant">
                            ...
                            <div class="flex items-center justify-between p-md bg-surface-container-lowest rounded-lg border border-outline-variant">
                                <div class="flex items-center gap-md">
                                    <span class="w-3 h-3 rounded-full animate-pulse" style="background-color: {{ $syncStatus['status_color'] }}"></span>
                                    <div>
                                        <p class="text-body-md font-bold">{{ $syncStatus['message'] }}</p>
                                        @if($syncStatus['last_sync'])
                                            <p class="text-xs text-on-surface-variant">Dernière mise à jour : {{ Carbon::parse($syncStatus['last_sync'])->diffForHumans() }}</p>
                                        @endif
                                    </div>
                                </div>
                                <span class="px-sm py-xs rounded-full text-xs font-bold" style="background-color: {{ $syncStatus['status_color'] }}10; color: {{ $syncStatus['status_color'] }}">{{ $syncStatus['status'] }}</span>
                            </div>
                        </div>
                    </div>
    ...
    <nav class="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-xs pb-safe h-20 bg-surface dark:bg-inverse-surface shadow-lg rounded-t-xl">
        <a class="flex flex-col items-center justify-center text-on-surface-variant dark:text-surface-variant px-4 py-1" href="{{ route('admin.dashboard') }}">
            <span class="material-symbols-outlined">qr_code_scanner</span>
            <span class="font-label-md text-label-md">Scanner</span>
        </a>
        <a class="flex flex-col items-center justify-center text-on-surface-variant dark:text-surface-variant px-4 py-1" href="{{ route('admin.orders.index') }}">
            <span class="material-symbols-outlined">history</span>
            <span class="font-label-md text-label-md">Historique</span>
        </a>
        <a class="flex flex-col items-center justify-center bg-primary-container dark:bg-primary text-on-primary-container dark:text-on-primary rounded-xl px-4 py-1" href="{{ route('admin.settings') }}">
            <span class="material-symbols-outlined">person</span>
            <span class="font-label-md text-label-md">Profil</span>
        </a>
        <a class="flex flex-col items-center justify-center text-on-surface-variant dark:text-surface-variant px-4 py-1" href="{{ route('admin.support') }}">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function refreshSyncStatus() {
                fetch('{{ route("admin.settings.shopify-sync-status") }}', {
                    headers: {
                        'Accept': 'application/json',
